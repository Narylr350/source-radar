...
All search tools support `page` (pagination), `nocache` (skip cache), and automatic quality assessment. When quality is low, the response includes `⚠️` warnings and `💡` suggestions.

`search_chinese_platforms` accepts `platforms` parameter to select specific platforms (xhs, wb, bili, tieba, dy, zhihu). Without it, defaults to 1 active platform. Requires MediaCrawler bridge running.
...
If the user's AI tool supports MCP (Claude Code, MiMoCode, Cursor), help them configure it:

**Claude Code** — add to `~/.claude.json` under `mcpServers`:
```json
"source-radar": {
  "command": "<full-path-to-uv>",
  "args": ["run", "--extra", "mcp", "--directory", "<project-path>", "python", "-m", "source_radar", "mcp"],
  "type": "stdio"
}
```

**MiMoCode** — add to `~/.config/mimocode/mimocode.json`:
```json
"source-radar": {
  "type": "local",
  "command": ["uv", "run", "--extra", "mcp", "--directory", "<project-path>", "source-radar", "mcp"],
  "enabled": true,
...
After MCP is configured, add usage instructions to your AI tool's global instructions so it automatically uses source-radar for web searches. See `README.md` → "AI Agent 全局指令" section for templates.
