...
- `systematic-debugging`：用于后端空结果、cookie、限流、安装失败和不稳定行为诊断。
- `codebase-design`：用于采集内核、backend seam、installer/lifecycle 边界设计。
- `test-driven-development`：用于新增 backend、installer、registry、lifecycle 或修 bug，先写失败测试。
