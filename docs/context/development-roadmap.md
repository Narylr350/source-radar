...
- M5: MediaCrawler source-acquisition foundation（已建立）
  - 真实搜索 / 来源发现已接入 `search` provider，普通 claim 不再主要依赖 fixture fallback
  - crawler/search provider interface 已统一内置 adapter、MediaCrawler bridge 的采集语义
  - MediaCrawler 等受限许可证项目只能走 external/local bridge，不 vendor 源码
  ...
  - MediaCrawler bridge 通过 `source-radar.bridge.v1` manifest 暴露 capabilities 和 AI guidance，供内置 agent 自动选择
  - `source-radar bridge mediacrawler` 提供第一方本机 bridge 运行器，调用用户本地 MediaCrawler WebUI API
  ...
  - README 明确手动安装、未来自动下载、预打包分发时必须保留 MediaCrawler 的 upstream license、version、source URL、NOTICE 和许可证义务
- M6: 综合信息分析 agent（已建立）
...
v1 不做 Web UI 或桌面 UI；不做人工手动补证据入口；不绕过登录、破解访问控制、反检测或验证码；不做大规模批量采集或监控；不直接 vendor MediaCrawler 源码；不把 LLM 判断伪装为最终事实裁定。
...
- integrations: 配置可选外部 MediaCrawler 兼容 bridge
